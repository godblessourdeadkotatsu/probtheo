...
\begin{lemma}
	Let ${(x_n)}_{n}$ be a sequence of positive real numbers and let $\bar{x}_n=\frac{\sum_{i=1}^{n}x_i}{n}$. Let $N={(n_k)}_{k}$ be a subsequence of $\N$ with 
	\[\lim_k\frac{n_{k+1}}{n_k}=r>0.\]
	If ${(\bar{x}_n)}_{n}$ converges along $N$ to $x$ then
...
\begin{fancyproof}
	...
	\[\pr(\Omega_0)=\pr(\lim_k\left\{N_k<+\infty\right\})=\underbracket{\lim_k\pr(N_k<+\infty)}_{\text{sequence of 1}}\]
...
\begin{theorem}
	...
	\[\sum_n \ev B_n<+\infty\implies\sum_n\implies\sum_n B_n<+\infty\as\]
	\item If 
...
\begin{example}
	Consider
	\[\Omega=[0,1],\;\HS=\B_{[0,1]},\;\pr=\underbracket{\leb}_{\mathclap{\text{Lebesgue measure}}}.\]
...
\begin{fancyproof}
	...
	\[p_n=p_n(\varepsilon)=\ev[\underbracket{\mathcal{i}_\varepsilon\circ X_n}_{\mathclap{\text{Bernoulli r.v.}}}]=\pr(X_n>\varepsilon).\]
...
\end{proposition}
Instead of proving this versione, we will prove a slightly different version of the proposition.
\end{document} 
