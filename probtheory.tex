...
\end{tcolorbox}
	\begin{revise}
		...
		In Riemann integration, $\dx$ represents an infinitesimal segment of the $x$-axis multiplied by the height of the function at $x$ (which is, of course, $f(x)$) and summed ($\int_{a}^{b}$) with all the other infinitesimal segments of the $x$-axis over the interval $[a,b]$. Here it's really the same thing with the difference that we do multiply the height of the function $f(x)$ by calculating the "weight", or "\underline{measure}" of the infinitesimal part of the domain of the function $\dx$ according to our method of measure of choice. We do this over the set $E$.\par	
	\end{revise}
	...
	Anyway was it really SO DIFFICULT to explain?
\begin{definition}
...
\begin{example}
	...
	We have an answer for this, but we need to do a bit of trackbacking.
	\begin{revise}
		First of all: what exactly \textit{means} to be absolutely continuous?
		\begin{definition}
		...
		\begin{theorem}
			...
			\[\frac{\nu(\dx)}{\mu(dx)}=\frac{\dif\nu}{\dif\mu}=p.\]
		\end{theorem}
		With all this alternative notation this thing honestly feels like trying to understand the Metal Gear Solid plot, where identical characters named Snake keep cloning each other and being triple crossed by everyone until you lose track of your own identity.
	\end{revise}
...
\end{remark}
\end{revise}
Yeah, that mysterious measure I was talking about before to connect Riemann and Lebesgue integration was the Lebesgue measure. We often just write $\dx$ to express the Lebesgue measure (which is what we did on example 1.2 about the exponential random variable).\\
...
\begin{example}
	...
	\begin{remark}
		...
		In this case $K(x,\dy)$ cannot be the distribution fo $Y$, because it has some $x$ in it. So this distribution is not simply the product of marginal distribution. But let's take a closer look to the form $\mu(\dx)K(x,\dy)$. $\mu(\dx)$ is certaninly a measure, but what about $K(x,\dy)$? \par
		...
		$K(x,\dy)$ has the form 
		\[B\mapsto K(x,B):\]
		It should be seen as a set function (after all we are constructing a \underline{join}t density, which is a set function in itself.)
	\end{remark}
	Why?
\end{example}
\end{document}