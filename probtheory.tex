...
\begin{remark}
	\begin{enumerate}[\circnum]
		...
		\end{fancyproof}
		\item If $K$ is $L^{p}$-bounded with $p>1$ then is is uniformly integrable. To prove this we recur to the following proposition:
		\begin{proposition}
			Suppose it exists a positive borel function $f$ on $\R_+$ such that
			\[\lim_{x\to\infty}\frac{f(x)}{x}=\infty\]
			and write 
			\[c=\sup_{X\in K}\ev f\circ|X|.\]
			If $x\leq \infty$ then $K$ is uniformly integrable.
		\end{proposition}
	\end{enumerate}
\end{remark}
\end{document} 
